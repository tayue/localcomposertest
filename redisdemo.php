...
abstract class BloomFilterRedis
{
    ...
    public function __construct()
    {
        ...
        $this->Redis = self::_redis(); //假设这里你已经连接好了
    }
    ...
    public static function _redis()
    {
      if (empty(self::$redis)) {
            ...
            try {
                $ret = self::$redis->connect("localhost", 6379);
                if ($ret === false) {
    ...
    public function add($string)
    {
      //  $pipe = $this->Redis->multi();
        foreach ($this->hashFunction as $function) {
            $hash = $this->Hash->$function($string);var_dump($this->bucket,$function,$hash);
...
$f=new FilteRepeatedComments();
$f->add("helloworld");
//var_dump($f->exists("sdfasdfasdfasdfadsf"));
